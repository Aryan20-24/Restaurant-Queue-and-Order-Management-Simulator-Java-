...
class BillingHeap{
    ...
    void rebuildDownAdd(){
        ...
        while(2*i<=num){
            ...
            if(comp(heap[i], heap[smallIndex])>0){
                ...
                heap[smallIndex]= tmp;
                heapt[heap[i].researchex].researchex= i;
                heapt[heap[smallIndex].researchex].researchex=smallIndex;
                i= smallIndex;
            }
    ...
    void rebuildDownRemove(){
        ...
        while(2*i<=num){
            ...
            if(compt(heapt[i],heapt[smallIndex])>0){
                ...
                heap[heapt[i].researchex].researchex= i;
                heap[heapt[smallIndex].researchex].researchex=smallIndex;
                i= smallIndex;
            }
    ...
    void rebuildUpAdd(int i){
        while(i>1){
            if(compt(heapt[i], heapt[i/2])<0){
                ...
                heapt[i/2]= tmp;
                heap[heapt[i].researchex].researchex=i;
                heap[heapt[i/2].researchex].researchex=i/2;
                i=i/2;
            }
            else{
                break;
    ...
    void rebuildUpRemove(int i){
        while(i>1){
            if(comp(heap[i], heap[i/2])<0){
                ...
                heap[i/2]= tmp;
                heapt[heap[i].researchex].researchex=i;
                heapt[heap[i/2].researchex].researchex=i/2;
                i=i/2;
            }
            else{
                break;
    ...
    void addCustomer(Node n){
        heap[1].put(n);
        heapt[heap[1].researchex].put(n);
        rebuildUpAdd(heap[1].researchex);
        rebuildDownAdd();
    }

    Node removeCustomer(){
        Node tmp=heapt[1].remove();
        heap[heapt[1].researchex].remove();
        rebuildUpRemove(heapt[1].researchex);
        rebuildDownRemove();
        return tmp;
    }
...
public class MMBurgers implements MMBurgersInterface {
    ...
    public void arriveCustomer(int id, int t, int numb) throws IllegalNumberException{
	    ...
	    if(t<clock || numb<=0){
            ...
            clock=t;
            simulate(prevTime, clock);
            Node customer= new Node(id, numb, t);
            bh.addCustomer(customer);
            tree.root=tree.insert(tree.root, customer);
            totalcus= tota+1;
        }
    } 
	...
	public float avgWaitTime(){
        //your implementation
        // float tmp= (float)tree.waitSum(tree.root)/tota;
        // System.out.println("average wait time is: "+tmp);
	    return (float)tree.waitSum(tree.root)/tota;
    } 
    ...
    private void simulate(int start, int end){
        for(int time=start; time<=end; ++time){
            while(!bh.isEmpty() && bh.topt().orderTime== time){
                ...
                tree.root=tree.update(tree.root, tmp);
            ...
            while(!deliver.isEmpty() && deliver.get(0).deliveryTime==time){
                ...
                tree.root= tree.update(tree.root, tmp1);
            }
